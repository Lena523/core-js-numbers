...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distanceBetweenPoints = Math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2);
...
function getLinearEquationRoot(a, b) {
  const linearEquationRoot = b < 0 ? Math.abs(b) / a : -b / a;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalMultOfvectors = x2 * x1 + y2 * y1;
  const modulA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modulB = Math.sqrt(x2 ** 2 + y2 ** 2);
  const angleBetweenVectors = Math.acos(scalMultOfvectors / (modulA * modulB));
...
function getParallelepipedDiagonal(a, b, c) {
  const parallelepipedDiagonal = Math.sqrt(a ** 2 + b ** 2 + c ** 2);
...
function roundToPowerOfTen(num, pow) {
  const roundIndex = 10 ** pow;
  const roundOfTen = Math.round(num / roundIndex) * roundIndex;
  return roundOfTen;
}
...
function getSine(num) {
  const sinOfNumber = Math.sin(num);
  return sinOfNumber;
}
...
function numberToStringInBase(number, base) {
  const numToChange = number.toString(base);
  return numToChange;
}
...
function toExponential(number, fractionDigits) {
  const numToTransform = number.toExponential(fractionDigits);
...
function toFixed(number, fractionDigits) {
  const numFixed = number.toFixed(fractionDigits);
  return numFixed;
}
...
function toPrecision(number, precision) {
  const numPrecision = number.toPrecision(precision);
  return numPrecision;
}
...
function isInteger(number) {
  const numBool = Number.isInteger(number);
  return numBool;
}
...
function getFloatOnString(str) {
  const numFloat = Number.parseFloat(str);
  return numFloat;
}
...
function getIntegerOnString(str, base) {
  const numParseInt = Number.parseInt(str, base);
  return numParseInt;
}
...
function isSafeInteger(number) {
  const numSafe = Number.isSafeInteger(number);
  return numSafe;
}
...
function roundToSmallestInteger(number) {
  const numSmallest = Math.floor(number);
  return numSmallest;
}
...
function roundToLargestInteger(number) {
  const numBiggest = Math.ceil(number);
  return numBiggest;
}
...
function getIntegerPartNumber(number) {
  const onlyInt = Math.trunc(number);
  return onlyInt;
}
...
function getMaxNumber(firstNumber, secondNumber) {
  const theLargest = Math.max(firstNumber, secondNumber);
  return theLargest;
}
...
function getRandomInteger(min, max) {
  ...
  const numRand = Math.floor(Math.random() * (numMax - numMin) + numMin);
  return numRand;
}
